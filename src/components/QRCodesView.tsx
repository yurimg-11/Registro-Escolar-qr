...
export default function QRCodesView() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      setSelectedFolios(new Set(data.map((s: Student) => s.folio)));
    } catch (error) {
  ...
  const toggleAll = () => {
    if (selectedFolios.size === filteredStudents.length) {
      setSelectedFolios(new Set());
    } else {
      setSelectedFolios(new Set(filteredStudents.map(s => s.folio)));
    }
  };
  ...
  const selectedStudents = students.filter(s => selectedFolios.has(s.folio));
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-slate-700">
                Alumnos ({selectedFolios.size}/{filteredStudents.length})
              </label>
              <button 
                onClick={toggleAll}
                className="text-xs text-indigo-600 hover:text-indigo-700 font-medium"
              >
                {selectedFolios.size === filteredStudents.length ? 'Deseleccionar' : 'Seleccionar todos'}
              </button>
            </div>
                ...
                filteredStudents.map(student => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-slate-500 truncate">{student.group_name} • {student.folio}</p>
                    </div>
                  </button>
                ))
              )}
      ...
      <div className="flex-1 bg-white p-8 rounded-2xl shadow-sm border border-slate-200 print:p-0 print:border-none print:shadow-none print:bg-transparent">
        {selectedStudents.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-slate-500 print:hidden min-h-[400px]">
            ...
            <p>Selecciona al menos un alumno para generar su código QR</p>
          </div>
        ) : (
          <div className={`
              ...
              >
                {template === 'id-card' && (
                  <div className="flex flex-col items-center p-6 border-2 border-slate-200 rounded-2xl bg-white text-center h-full print:border-black print:rounded-lg">
                    <div className="w-full bg-indigo-600 text-white py-2 rounded-t-xl -mt-6 mb-6 print:bg-black print:text-white print:rounded-t-md print:-mt-6 print:mb-4">
                      <h3 className="font-bold tracking-widest uppercase text-sm">Escuela</h3>
                    </div>
                    ...
                    <h4 className="text-lg font-bold text-slate-900 leading-tight mb-1">{student.name}</h4>
                    <p className="text-sm font-medium text-slate-500 mb-6">Grupo: {student.group_name}</p>
                    <div className="mt-auto bg-white p-2 rounded-xl border border-slate-100 print:border-none">
                      <QRCode value={student.folio} size={120} level="H" />
                    </div>
                    <p className="text-xs font-mono text-slate-400 mt-2">{student.folio}</p>
                  </div>
                )}
                ...
                {template === 'sticker' && (
                  <div className="flex items-center p-4 border-2 border-slate-200 rounded-xl bg-white print:border-black print:rounded-md">
                    <div className="bg-white p-1 mr-4 flex-shrink-0">
                      <QRCode value={student.folio} size={80} level="M" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-base font-bold text-slate-900 truncate">{student.name}</h4>
                      <p className="text-sm text-slate-600">Grupo: {student.group_name}</p>
                      <p className="text-xs font-mono text-slate-400 mt-1">{student.folio}</p>
                    </div>
                  </div>
                )}
                ...
                {template === 'minimal' && (
                  <div className="flex flex-col items-center p-4 border border-slate-200 rounded-xl bg-white print:border-black print:rounded-none">
                    <QRCode value={student.folio} size={100} level="L" />
                    <p className="text-xs font-mono text-slate-900 mt-2 font-bold">{student.folio}</p>
                  </div>
                )}
              </div>
            ))}
